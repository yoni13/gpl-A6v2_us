...
#endif
	if (count%100 == 0)
		my_syslog(MS_DHCP | LOG_ERR, _("(%s, %d)==before daemon->dhcp=="), __FUNCTION__, __LINE__);
#ifdef HAVE_DHCP
      if (daemon->dhcp)
	{
...
#endif
	if (count%100 == 0)
		my_syslog(MS_DHCP | LOG_ERR, _("(%s, %d)==after daemon->dhcp=="), __FUNCTION__, __LINE__);
...
#    endif
	if (count%100 == 0)
		my_syslog(MS_DHCP | LOG_ERR, _("(%s, %d)==before do_script_run=="), __FUNCTION__, __LINE__);
...
#  else
	...
	if (count%100 == 0)
		my_syslog(MS_DHCP | LOG_ERR, _("(%s, %d)==after do_script_run=="), __FUNCTION__, __LINE__);
...
#  endif
	...
	if (count%100 == 0)
		my_syslog(MS_DHCP | LOG_ERR, _("(%s, %d)==before select=="), __FUNCTION__, __LINE__);
	...
	if (count%100 == 0)
		my_syslog(MS_DHCP | LOG_ERR, _("(%s, %d)==after select=="), __FUNCTION__, __LINE__);
	...
	{
		if (count%100 == 0)
			my_syslog(MS_DHCP | LOG_ERR, _("(%s, %d)==is dad listeners?=="), __FUNCTION__, __LINE__);
...
#endif
	if (count%100 == 0)
		my_syslog(MS_DHCP | LOG_ERR, _("(%s, %d)==before dhcpfd 222=="), __FUNCTION__, __LINE__);
      ...
      if (daemon->dhcp)
	{
	  ...
	  {
	  	if (count%100 == 0)
			my_syslog(MS_DHCP | LOG_ERR, _("(%s, %d)==in dhcpfd 222=="), __FUNCTION__, __LINE__);
	    dhcp_packet(now, 0);
	  }
	  if (daemon->pxefd != -1 && FD_ISSET(daemon->pxefd, &rset))
	  {
		if (count%100 == 0)
			my_syslog(MS_DHCP | LOG_ERR, _("(%s, %d)==in dhcpfd 333=="), __FUNCTION__, __LINE__);
	    dhcp_packet(now, 1);
	  }
	}
	  if (count%100 == 0)
	  my_syslog(MS_DHCP | LOG_ERR, _("(%s, %d)==after dhcpfd 222=="), __FUNCTION__, __LINE__);
...
#  endif
	if (count%100 == 0)
	{
		my_syslog(MS_DHCP | LOG_ERR, _("(%s, %d)==the end==\n"), __FUNCTION__, __LINE__);
		count = 0;
	}
#endif
