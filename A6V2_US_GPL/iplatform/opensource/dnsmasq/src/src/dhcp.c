...
static int make_fd(int port)
{
  ...
  my_syslog(MS_DHCP | LOG_ERR, _("(%s, %d)==init dhcp sock =="), __FUNCTION__, __LINE__);
...
void dhcp_packet(time_t now, int pxe_fd)
{
  ...
  if (!iface_check(AF_INET, (struct all_addr *)addrp, ifr.ifr_name))
    {
	 ...
	 for a secondary */
      ...
      my_syslog(MS_DHCP | LOG_ERR, _("(%s, %d)==11111=="), __FUNCTION__, __LINE__);
      if (!daemon->if_addrs ||
  ...
  my_syslog(MS_DHCP | LOG_ERR, _("(%s, %d)==22222=="), __FUNCTION__, __LINE__);
