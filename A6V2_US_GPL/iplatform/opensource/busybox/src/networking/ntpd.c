...
step_time(double offset)
{
...
#ifdef NTP_TIME_GET_ENABLED
	fd = open("/tmp/ntp_time_set", O_RDONLY);
	if(fd < 0)
	{
		fd = open("/tmp/ntp_time_set", O_RDWR | O_CREAT | O_TRUNC);
		if (fd < 0 )
		{
			system("echo \"========>create ntp_time_set failed\" > /dev/console");
		}
		else
		{
	...
	{
		system("echo \"========>ntp_time_set exits\" > /dev/console");
		close(fd);
	}
