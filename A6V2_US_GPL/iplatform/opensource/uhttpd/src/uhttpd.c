...
static char* new_session_urls[] = {
	"/webpages/login.html",	
	"/webpages/index%s.html",		/* access when reflesh */
	NULL
};
...
static bool uh_new_session_url_match(char *url, char *target_uls[], bool match_root)
{
	...
	while(target_uls[index])
	{
		...
		//if(strstr(url, target_uls[index]))
...
static bool uh_check_https_redirect(struct client *cl, struct http_request *req)
{
    ...
    if(conf->tls && cl->server && !cl->server->tls)
    {
        ...
        if((!cl->local_access && uh_new_session_url_match(req->url, NULL, true)) 
			|| (cl->local_access && cl->local_https_on
					&& uh_new_session_url_match(req->url, new_session_urls, true)))
        {
            /* get HOST */
...
static bool uh_check_https_disabled(struct client *cl, struct http_request *req)
{
	struct config *conf = cl->server->conf;
    ...
    if(conf->tls && cl->server && cl->server->tls)
    {
        ...
        {           
			...
			//						"Local Management via HTTPS is disable.");
			sprintf(path_phys, "%s%s", conf->docroot, UH_HTTPS_403PAGE);
...
#ifdef HAVE_TLS		
        if(uh_check_https_disabled(cl, req))
        {
        	return false;
        }       
		
        /* add http redict check, wl, 2017-09-21 */		
        if(uh_check_https_redirect(cl, req))
        {
        	return false;
        }
        /* add ended */
