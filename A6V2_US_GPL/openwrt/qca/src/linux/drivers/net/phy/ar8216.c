...
static void ar8327_hw_reset(struct ar8216_priv *priv)
{
	int i;
	u32 reg[2] = {0xe0, 0x10};
	u32 value[2] = {0xc74164de, 0x602613a0};

	for (i = 0; i < 2; i++) {
		if (reg[i]) {
			printk("before HW reset reg[%x]: [%x] -> [%x]\n", reg[i], priv->read(priv, reg[i]), value[i]);
		}
	}
	gpio_set_value(11, 0);
	udelay(1000 * 1000);
	...
	for (i = 0; i < 2; i ++) {
		if (reg[i]) {
			printk("recovery reg[%x]: [%x] -> [%x]\n", reg[i], priv->read(priv, reg[i]), value[i]);
			priv->write(priv, reg[i], value[i]);
		}
	}
}
#endif
...
static int qca_qm_error_check(struct ar8216_priv *priv)
{
	...
	value = priv->read(priv, 0xe0);
	if (value != 0xc74164de) {
		printk("wrong reg[0xe0] is 0x%x, but it should be 0x%x\n", value, 0xc74164de);
		value = priv->read(priv, 0xe0);
		if (value != 0xc74164de) {
			printk("wrong reg[0xe0] is 0x%x, but it should be 0x%x\n", value, 0xc74164de);
			return 1;
...
#ifdef AR8216_PHY_RESET_CMD
void (*ar8216_hnat_init_switch_reg)(void);
EXPORT_SYMBOL(ar8216_hnat_init_switch_reg);
#endif
static int qca_qm_err_recovery(struct ar8216_priv *priv)
{
#ifdef AR8216_PHY_RESET_CMD
	void (*hnat_init_reg)(void);
#endif
...
#ifdef AR8216_PHY_RESET_CMD
	rcu_read_lock();
	hnat_init_reg = rcu_dereference(ar8216_hnat_init_switch_reg);
	if (hnat_init_reg) {
		hnat_init_reg();
	}
	rcu_read_unlock();
...
ar8216_config_init(struct phy_device *pdev)
{
	...
	printk(KERN_INFO "%s: %s switch phy power down.\n", pdev->attached_dev->name, swdev->name);

	// ar8xxx_phy_enable(priv); don't enable phy when booting...
...
#ifdef AR8216_PHY_RESET_CMD
static ssize_t ar8216_hw_reset(struct device *dev,
...
struct kobject *ar8216_sys = NULL;
static const struct device_attribute ar8216_hw_reset_attr = 
	__ATTR(hw_reset, 0660, NULL, ar8216_hw_reset);
#endif
...
{
#ifdef AR8216_PHY_RESET_CMD
	ar8216_sys = kobject_create_and_add("ar8216_sys", NULL);
	if (!ar8216_sys) {
		printk("ar8216 failed to register sysfs\n");
	} else {
		sysfs_create_file(ar8216_sys, &ar8216_hw_reset_attr.attr);
	}
#endif
...
#ifdef AR8216_PHY_RESET_CMD
	if (ar8216_sys) {
		sysfs_remove_file(ar8216_sys, &ar8216_hw_reset_attr.attr);
		kobject_put(ar8216_sys);
	}
#endif
}
