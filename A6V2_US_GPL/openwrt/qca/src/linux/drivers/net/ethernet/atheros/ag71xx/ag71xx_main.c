...
#endif
#define AR8216_PHY_RESET_CMD
#define CONFIG_AG71XX_DROP_PAUSE_IN_DRIVER 1
#ifdef CONFIG_AG71XX_DROP_PAUSE_IN_DRIVER
static inline int is_link_local(const unsigned char *dest) {
    u8 br_group_address[ETH_ALEN] = { 0x01, 0x80, 0xc2, 0x00, 0x00, 0x00 };
    __be16 *a = (__be16 *)dest;
    __be16 *b = (__be16 *)br_group_address;
...
#define FIFO_CFG4_INIT	(FIFO_CFG4_DE | FIFO_CFG4_DV | FIFO_CFG4_FC | \
			 ...
			 FIFO_CFG4_BC | FIFO_CFG4_DR | FIFO_CFG4_CF | \
			 FIFO_CFG4_PF | FIFO_CFG4_UO | FIFO_CFG4_VT | \
			 FIFO_CFG4_LE | FIFO_CFG4_FT | FIFO_CFG4_UC)

#define FIFO_CFG5_INIT	(FIFO_CFG5_DV | FIFO_CFG5_LM | FIFO_CFG5_LO | \
			 FIFO_CFG5_OK | FIFO_CFG5_MC | FIFO_CFG5_BC | \
			 FIFO_CFG5_CF | FIFO_CFG5_UO | FIFO_CFG5_VT | \
			 FIFO_CFG5_UC | FIFO_CFG5_SF | FIFO_CFG5_FC)
...
#ifdef CONFIG_AG71XX_DROP_PAUSE_IN_DRIVER
	ag71xx_wr(ag, AG71XX_REG_FIFO_CFG5, FIFO_CFG5_INIT | FIFO_CFG5_CF | FIFO_CFG5_PF);
#else
...
void ag71xx_link_adjust(struct ag71xx *ag)
{
	...
	if (!ag->link) {
		...
		if (netif_msg_link(ag))
			pr_info("%s: link down\n", ag->dev->name);
#ifdef CONFIG_AG71XX_CHECK_RX_STUCK
		cancel_delayed_work(&ag->txrx_stuck.work);
#endif
		return;
	}
	...
	DBG("%s: mac_cfg2=%#x, mac_ifctl=%#x\n",
		...
		ag71xx_rr(ag, AG71XX_REG_MAC_CFG2),
		ag71xx_rr(ag, AG71XX_REG_MAC_IFCTL));
#ifdef CONFIG_AG71XX_CHECK_TX_STUCK
	ag->txrx_stuck.tx_pause = 0;
	ag->txrx_stuck.tx_pause_cnt = 0;
#endif
#ifdef CONFIG_AG71XX_CHECK_RX_STUCK
	ag->txrx_stuck.rx_pause = 0;
	ag->txrx_stuck.rx_fcs_cnt = 0;
	ag->txrx_stuck.rx_int_cnt = 0;
	ag->txrx_stuck.prev_rx_int_cnt = 0;
	ag->txrx_stuck.rx_packet = 0;
	ag->txrx_stuck.mib_rx_packet = 0;
        schedule_delayed_work(&ag->txrx_stuck.work, msecs_to_jiffies(1000));
#endif
}
...
static int ag71xx_open(struct net_device *dev)
{
	...
	int ret;
#ifdef CONFIG_AG71XX_CHECK_TXRX_STUCK
	unsigned long flags;
	struct ag71xx_platform_data *pdata = ag71xx_get_pdata(ag);
	u32 reset_mask = pdata->reset_bit;
...
#ifdef CONFIG_AG71XX_CHECK_TX_STUCK
	if (ag->tx_ring.dirty->desc->ctrl & DESC_EMPTY) {
		pr_info("%s: wait kernel to recycle tx ring\n", ag->dev->name);
		dev->trans_start = jiffies;
		return;
	}
	if (!ag->txrx_stuck.tx_pause) {
		pr_info("%s: check pause frame count\n", ag->dev->name);
		ag->txrx_stuck.tx_pause = 1;
		ag->txrx_stuck.tx_pause_cnt = ag71xx_rr(ag, 0xB4); //0xb4: GMAC received pause frame count
	} else if (ag->txrx_stuck.tx_pause_cnt == ag71xx_rr(ag, 0xB4)) {
...
#ifndef CONFIG_AG71XX_CHECK_TXRX_STUCK
...
#ifndef CONFIG_AG71XX_CHECK_RX_STUCK
...
#else
static void ag71xx_check_rx_stuck(struct work_struct *work)
{
	struct ag71xx *ag = container_of(work, struct ag71xx, txrx_stuck.work.work);

	if (netif_running(ag->dev) && netif_carrier_ok(ag->dev)) {
		// Check RX stuck or not
		if (! ag->txrx_stuck.rx_pause) {
			if ((ag->txrx_stuck.prev_rx_int_cnt != ag->txrx_stuck.rx_int_cnt) ||
			    ((ag->txrx_stuck.rx_fcs_cnt == ag71xx_rr(ag, 0xA4)) && (ag->txrx_stuck.mib_rx_packet != ag71xx_rr(ag, 0xA0)))) {
				if (ag->txrx_stuck.rx_packet == ag->dev->stats.rx_packets) {
					ag->txrx_stuck.rx_pause = 1;
				}
			}
		} else if (ag->txrx_stuck.rx_packet == ag->dev->stats.rx_packets) {
			printk("ERROR: dev(%s) rx stuck\n", ag->dev->name);
			schedule_work(&ag->restart_work);
			return;
		}
		ag->txrx_stuck.prev_rx_int_cnt = ag->txrx_stuck.rx_int_cnt;
		ag->txrx_stuck.mib_rx_packet = ag71xx_rr(ag, 0xA0);
		ag->txrx_stuck.rx_fcs_cnt = ag71xx_rr(ag, 0xA4);
		if (! ag->txrx_stuck.rx_pause) {
			ag->txrx_stuck.rx_packet = ag->dev->stats.rx_packets;
		}
		schedule_delayed_work(&ag->txrx_stuck.work, msecs_to_jiffies(1000));
	}
}
...
#ifdef CONFIG_AG71XX_DROP_PAUSE_IN_DRIVER
static void ag71xx_rx_replenish(struct ag71xx *ag, struct sk_buff_head *skb_pause_list)
#else
...
#ifdef CONFIG_AG71XX_DROP_PAUSE_IN_DRIVER
		if (skb_pause_list) {
			dirty->skb = __skb_dequeue(skb_pause_list);
		}
		if (! dirty->skb) {
#endif
...
#else
#ifdef CONFIG_AG71XX_DROP_PAUSE_IN_DRIVER
	bool bSkbListInit = false;
	struct sk_buff_head skb_pause_list;
	unsigned int rx_buf_size = ag->rx_buf_size;
#endif
#endif
	int received = 0;
...
#ifdef CONFIG_AG71XX_RX_NO_REPLENISH
#ifdef CONFIG_AG71XX_DROP_PAUSE_IN_DRIVER
                if ((*((uint16_t *)&skb->data[12]) == htons(ETH_P_PAUSE)) && is_link_local((unsigned char *)skb->data)) {
			//printk("receive pause frame\n");
			new_skb = skb;
			skb = NULL;
                } else {
#endif
...
#ifdef CONFIG_AG71XX_DROP_PAUSE_IN_DRIVER
                if ((*((uint16_t *)&skb->data[12]) == htons(ETH_P_PAUSE)) && is_link_local((unsigned char *)skb->data)) {
			//printk("receive pause frame 1\n");
			if (!bSkbListInit) {
				__skb_queue_head_init(&skb_pause_list);
				bSkbListInit = true;
			}

			__skb_queue_tail(&skb_pause_list, skb);
			skb = NULL;
		}
#endif
#endif
...
#ifdef CONFIG_AG71XX_DROP_PAUSE_IN_DRIVER
		if (pktlen > rx_buf_size) {
			printk("ERROR: rx packet length is %d\n", pktlen);
...
#ifdef CONFIG_AG71XX_DROP_PAUSE_IN_DRIVER
	if (bSkbListInit) {
		ag71xx_rx_replenish(ag, &skb_pause_list);
	} else {
		ag71xx_rx_replenish(ag, NULL);
	}
#else
...
#ifdef CONFIG_AG71XX_CHECK_RX_STUCK
	INIT_DELAYED_WORK(&ag->txrx_stuck.work, ag71xx_check_rx_stuck);
#endif
