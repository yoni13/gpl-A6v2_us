...
static void a9v6SchemeInit(void)
{
	ath_reg_rmw_set(GPIO_OE_ADDRESS,  (1 << 16)); /* use FactoryResetButton(GPIO16) as input indicator */
	ath_reg_rmw_clear(GPIO_OE_ADDRESS,(1 << 1));/* use WPSLED(GPIO1) as output indicator */
}

static int a9v6KeyPressed(void)
{
    int ret = 0;
    ret = getKeyValueByGPIO(16);   /* a9v6 use FactoryResetButton(GPIO16) as FW Recovery Button */
    if (ret < 0) return 0;        /* No Key Pressed Dectected. */
    return (1-ret);               /* When the key is pressed, the key value is 0 */
}

static void a9v6TurnLedOn(void)
{
    /* Clear GPIO 1 to turn on WPS LED. */
    ath_reg_rmw_clear(GPIO_OUT_ADDRESS, (1 << 1));
}

static void a9v6TurnLedOff(void)
{
    /* Set GPIO 1 to turn off WPS LED. */
    ath_reg_rmw_set(GPIO_OUT_ADDRESS, (1 << 1));
}
...
static FW_RECOVERY_GPIO_SCHEME FWRecoveryA9V6Scheme =
{
    .schemeName   = "A9V6",
    .schemeInit   = NULL,
    .isKeyPressed = NULL,
...
#if defined(CONFIG_PRODUCT_A6V2)
static void a6v2SchemeInit(void)
{
	ath_reg_rmw_set(GPIO_OE_ADDRESS,  (1 << 5)); /* use FactoryResetButton(GPIO1) as input indicator */
	ath_reg_rmw_clear(GPIO_OE_ADDRESS,(1 << 1));/* use WPSLED(GPIO1) as output indicator */
}

static int a6v2KeyPressed(void)
{
    int ret = 0;
    ret = getKeyValueByGPIO(5);   /* c7v4/c7v5 use FactoryResetButton(GPIO5) as FW Recovery Button */
    if (ret < 0) return 0;        /* No Key Pressed Dectected. */
...
static void a6v2TurnLedOn(void)
{
    /* Clear GPIO 1 to turn on WPS LED. */
    ath_reg_rmw_clear(GPIO_OUT_ADDRESS, (1 << 1));
}

static void a6v2TurnLedOff(void)
{
    /* Set GPIO 1 to turn off WPS LED. */
    ath_reg_rmw_set(GPIO_OUT_ADDRESS, (1 << 1));
}

static FW_RECOVERY_GPIO_SCHEME FWRecoveryA6V2Scheme =
{
    .schemeName   = "A6V2",
    .schemeInit   = NULL,
    .isKeyPressed = NULL,
...
#if defined(CONFIG_PRODUCT_A9V6)
	if (!strcmp(FWRecoveryA9V6Scheme.schemeName, FIRMWARE_RECOVERY_GPIO_SCHEME))
	{
		FWRecoveryA9V6Scheme.schemeInit = a9v6SchemeInit;
		FWRecoveryA9V6Scheme.isKeyPressed = a9v6KeyPressed;
		FWRecoveryA9V6Scheme.turnLedOn  = a9v6TurnLedOn;
		FWRecoveryA9V6Scheme.turnLedOff = a9v6TurnLedOff;
		FW_recovery_scheme = &FWRecoveryA9V6Scheme;
		return;
	}
#endif

#if defined(CONFIG_PRODUCT_A6V2)
	if (!strcmp(FWRecoveryA6V2Scheme.schemeName, FIRMWARE_RECOVERY_GPIO_SCHEME))
	{
		FWRecoveryA6V2Scheme.schemeInit = a6v2SchemeInit;
		FWRecoveryA6V2Scheme.isKeyPressed = a6v2KeyPressed;
		FWRecoveryA6V2Scheme.turnLedOn  = a6v2TurnLedOn;
		FWRecoveryA6V2Scheme.turnLedOff = a6v2TurnLedOff;
		FW_recovery_scheme = &FWRecoveryA6V2Scheme;
		return;
	}
#endif
