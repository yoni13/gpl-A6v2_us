...
static __inline__ int abortboot(int bootdelay)
{
	...
	int GPIO18_AS_UART_KEY_pressed = 0;
	int wantEnterSencondUbootCmd = 0;
#endif
...
#if defined(CONFIG_TP_FAST_BOOT)
			int key  = getc();
			if (key == CONFIG_ABORT_BOOT_KEY)
			{
				puts ("\b\b\b 0");
				abort = 1; 	/* don't auto boot	*/
			}
#if (defined(CONFIG_PRODUCT_A9V6) && defined(CFG_DOUBLE_BOOT_FACTORY)) 
			else if (key == CONFIG_GPIO18_AS_UART_KEY) {
				GPIO18_AS_UART_KEY_pressed = 1;
			}
			else if (key == CONFIG_ABORT_SECOND_BOOT_KEY) {
				wantEnterSencondUbootCmd = 1;
			}
#endif
#else
...
# endif
	...
	if (abort == 0)
	{
		if (GPIO18_AS_UART_KEY_pressed == 0
			&& wantEnterSencondUbootCmd == 0) {

			//select_gpio_18_func(GPIO18_USED_AS_WPS);
			printf("\n============ USE AS UART DEFAULT =============\n\n");
		}
		else {
			printf("\n============ WPS BTN disabled =============\n\n");
...
{	
	...
	int len = 0;
	unsigned char *buf = (char *)TEXT_BASE;
...
# endif	/* CONFIG_BOOT_RETRY_TIME */
	...
	if (bootlimit && (bootcount > bootlimit)) {
       ...
       if (!s) {
		...
		{
#if defined(CONFIG_TP_FAST_BOOT)
			abortboot_eth_initialize(gd->bd);
#endif
