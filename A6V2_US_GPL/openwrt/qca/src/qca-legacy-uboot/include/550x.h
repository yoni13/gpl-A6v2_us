 ...
 * With following optimiz, shorten two-stage U-Boot boot time from 8s to 2s,
 *
 * 1:
 * We do minimal initialization as far as possible.
 * eth is inited only when we actually need it (e.g. enter recovery stage)
 *
 * 2:
 * delay time is chagned from 1s to 0s, 
 * I record key press status to gd
 * so we can press key at any stage of uboot.
 *
 * 3 TODO
 * Check firmware integrity. only check the first min-io block.
 * when upgrade the firmware, the first min-io is always write last, 
 * so if the first block is right, all other blocks should be right.
 */
#define CONFIG_TP_FAST_BOOT

#if defined(CONFIG_PRODUCT_A9V6)
#define CONFIG_GPIO18_AS_UART_KEY	'a'
#define GPIO18_USED_AS_WPS		0
#define GPIO18_USED_AS_UART0SIN  1
#endif

#if defined(CONFIG_TP_FAST_BOOT)
#define CONFIG_ABORT_SECOND_BOOT_KEY '\r'
#if defined(CFG_DOUBLE_BOOT_FACTORY)
#define CONFIG_ABORT_BOOT_KEY	'\003'
#define CONFIG_MENUPROMPT 		"press CTRL+C to stop factory u-boot autoboot: %2d "
#else
#define CONFIG_ABORT_BOOT_KEY	CONFIG_ABORT_SECOND_BOOT_KEY
#define CONFIG_MENUPROMPT 		"press ENTER to stop autoboot: %2d "
#endif
#define CONFIG_BOOTDELAY 0
#define CONFIG_ZERO_BOOTDELAY_CHECK
#elif defined(CFG_DOUBLE_BOOT_FACTORY) || defined (CFG_DOUBLE_BOOT_SECOND)
