...
#if defined( CONFIG_PRODUCT_A9V6 ) || defined(CONFIG_PRODUCT_A6V2)
    #define WAN_PHY 0
...
void set_8337N_led(void)
{
#define PATTERN_OFF 0
#define PATTERN_BLINK 1
#define PATTERN_ON 2
#define PATTERN_MAP 3

	uint16_t rule = 	(PATTERN_MAP << 14)
					| 	(1 << 13) 	/* FD light */
					| 	(1 << 12) 	/* HD light */
					| 	(0 << 11) 	/* power on light */
					| 	(1 << 10) 	/* 1000Mbps light */
	...
	uint32_t rule2 = (rule << 16 ) | rule;
	
	athrs17_reg_write(S17_LED_CTRL0_REG, rule2);
	athrs17_reg_write(S17_LED_CTRL1_REG, rule2);
	athrs17_reg_write(S17_LED_CTRL2_REG, rule2);

	/* S17_LED_CTRL3_REG use default value */
...
athrs17_phy_setup(int ethUnit)
{
	...
	/* close lan phy too */
	phy_reg_write(ATHR_PHYBASE(LAN1_PHY), ATHR_PHYADDR(LAN1_PHY), ATHR_PHY_CONTROL, 0x0800);
	phy_reg_write(ATHR_PHYBASE(LAN2_PHY), ATHR_PHYADDR(LAN2_PHY), ATHR_PHY_CONTROL, 0x0800);
	phy_reg_write(ATHR_PHYBASE(LAN3_PHY), ATHR_PHYADDR(LAN3_PHY), ATHR_PHY_CONTROL, 0x0800);
	phy_reg_write(ATHR_PHYBASE(LAN4_PHY), ATHR_PHYADDR(LAN4_PHY), ATHR_PHY_CONTROL, 0x0800);

	if (!foundPhy) {
		return FALSE; /* No PHY's configured for this ethUnit */
	}

#if !defined(CONFIG_TP_FAST_BOOT) || defined(CFG_DOUBLE_BOOT_FACTORY)
	/*
...
void enable_phy_ports(void)
{
	printf("---------in uboot mode , enable phy -----\n");
	phy_reg_write(ATHR_PHYBASE(WAN_PHY), ATHR_PHYADDR(WAN_PHY), ATHR_PHY_CONTROL, 0x1200);
	phy_reg_write(ATHR_PHYBASE(LAN1_PHY), ATHR_PHYADDR(LAN1_PHY), ATHR_PHY_CONTROL, 0x1200);
	phy_reg_write(ATHR_PHYBASE(LAN2_PHY), ATHR_PHYADDR(LAN2_PHY), ATHR_PHY_CONTROL, 0x1200);
	phy_reg_write(ATHR_PHYBASE(LAN3_PHY), ATHR_PHYADDR(LAN3_PHY), ATHR_PHY_CONTROL, 0x1200);
	phy_reg_write(ATHR_PHYBASE(LAN4_PHY), ATHR_PHYADDR(LAN4_PHY), ATHR_PHY_CONTROL, 0x1200);
