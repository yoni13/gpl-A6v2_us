...
#define TP_RESET_SWITCH_GPIO 14
#else
#error TODO: define a correct switch reset gpio 
#endif

	if ( CFG_ATH_GMAC_NMACS == 1) {
		//   S17 SWITCH RESET
		val = ath_reg_rd(GPIO_OE_ADDRESS) & ~(1 << TP_RESET_SWITCH_GPIO);
		ath_reg_wr(GPIO_OE_ADDRESS, val);
		udelay(1000 * 100);
		ath_reg_rmw_set(GPIO_OUT_ADDRESS, ( 1 << TP_RESET_SWITCH_GPIO));
		udelay(1000 * 100);
		ath_reg_rmw_clear(GPIO_OUT_ADDRESS, ( 1 << TP_RESET_SWITCH_GPIO));
		udelay(1000 * 100);
		ath_reg_rmw_set(GPIO_OUT_ADDRESS, ( 1 << TP_RESET_SWITCH_GPIO));
	}
#endif
